#!/usr/bin/env python3
"""Fetch Qwen3-ASR GGUF weights into models/Qwen3-ASR-GGUF/.
...
continues where it stopped. Stdlib only; run from the repo root:
    ...
    python tools/audiocpp/fetch_qwen3_asr_gguf.py

Downloads the default 0.6B Q8 profile (~1.2 GiB, SHA256-verified single
...
from audiocpp_backend import REPO_ROOT as _ROOT, missing_specs, qwen3_asr_fetch_specs
...
def main() -> int:
    specs = qwen3_asr_fetch_specs(_ROOT / "models" / "Qwen3-ASR-GGUF")
    pending = missing_specs(specs)
    if not pending:
        print("Qwen3-ASR model file already present.")
        return 0
    try:
