...
// `mix <= 0` is a bit-exact memcpy bypass. All state is fixed member data;
// reset() clears biquad delay lines, the follower, and the GR estimate.
    ...
    void process(const float* in, float* out, int channels, int frames) {
        ...
        for (int i = 0; i < frames; ++i) {
            ...
            if (depth_db_ > 0.0f) {
                ...
                const float over = env_db - thresh_db_;
                if (over > 0.0f) {
