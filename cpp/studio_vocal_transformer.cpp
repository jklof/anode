...
//
// Architecture: real-time retune front end driving a spectral vocal
// pipeline (true-envelope phase vocoder with VTLN formant warp).
//
//   input -> F0 tracking -> retune (scale snap / MIDI target + glide)
//         -> vibrato -> resampled-frame pitch shift (pitch happens in the
//            frame read) -> 2048-pt True-Envelope estimation (Roebel-Rodet,
//            symmetric quefrency lifter, PCHIP peak hull) -> same-grid
//            peak-locked phase vocoder -> formant-preserving envelope
//            replacement + piecewise-linear knee VTLN warp (F1 decoupled;
//            log-octave spectral tilt, dynamic H1 boost) -> voiced/unvoiced
//            gated sibilant bypass -> tract-shaped aspiration noise ->
//            transient-gated phase reset -> ring-buffer OLA with FIXED
//            emission latency (kLatency = 9216 samples = 192 ms @ 48 kHz).
//
// Why spectral instead of TD-PSOLA: time-domain PSOLA resamples each grain,
// so formants shift WITH pitch (chipmunk: +10 st pitch drags formants +10 st
// instead of the +3 st a real shorter vocal tract gives), and grain
// placement needs glottal-closure landmarks that are ambiguous for harmonic-
// rich voice ( octave-up tracker locks, T0/2 peak flips, transient smearing
// -> chronic pops). The phase-vocoder path needs no period marks at all,
// shifts formants independently of pitch, and resets phase coherently on
// transients.
//
...
public:
    ...
    void set_samplerate(float sr) {
        ...
        // The pitch-tracker anti-alias filter tracks the rate too.
        if (sr > 1.0f && sr != sr_) {
            sr_ = sr;
            aa_filter_.set_lowpass(1200.0f, 0.7071f, sr_);
            recompute_tables();
